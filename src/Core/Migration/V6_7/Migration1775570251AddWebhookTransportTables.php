...
class Migration1775570251AddWebhookTransportTables extends MigrationStep
{
    ...
    public function update(Connection $connection): void
    {
        ...
        $this->createWebhookStreamTable($connection);
    ...
    private function createWebhookStreamTable(Connection $connection): void
    {
    ...
    private function addWebhookEventLogColumns(Connection $connection): void
    {
        ...
        if (!TableHelper::columnExists($connection, 'webhook_event_log', 'execution_count')) {
            $connection->executeStatement('
                ALTER TABLE `webhook_event_log`
                    ADD COLUMN `execution_count` INT UNSIGNED NOT NULL DEFAULT 0
            ');
        }

        if (!TableHelper::columnExists($connection, 'webhook_event_log', 'partition_key')) {
            $connection->executeStatement('
                ALTER TABLE `webhook_event_log`
                    ADD COLUMN `partition_key` BINARY(16) NULL
            ');
        }

        if (!TableHelper::columnExists($connection, 'webhook_event_log', 'last_attempt_at')) {
            $connection->executeStatement('
                ALTER TABLE `webhook_event_log`
